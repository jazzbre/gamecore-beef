...
namespace GameCore
{
    class CRTRenderer
    {
        ...
        hpos, vpos, hsize, vsize, contrast, brightness, saturation, blur, degauss;
        // range -1.0f to 1.0f, default=0.0f
        ...
        void RenderBlurCRT(RenderTexture source, RenderTexture target_a, RenderTexture target_b, float r, int width, int height)
        {
            ...
            {
                var commandBuffer = RenderManager.AcquireCommandBuffer(target_b);
                defer
                {
                    commandBuffer.Finish();
                    RenderManager.SubmitCommandBuffer(commandBuffer);
                }
                commandBuffer.ClearColorBuffer = false;
                commandBuffer.Viewport = .() { x = 0, y = 0, width = (uint16)width, height = (uint16)height };
                parameters.Blur = blur_vector;
                vertexbuffer.Render(commandBuffer, .Identity, blur_shader, scope GpuTexture[](source.TextureHandle), .White, state, sampler, parameters: .((uint8*)&parameters, sizeof(CrtParameters)));
            }
            {
                var commandBuffer = RenderManager.AcquireCommandBuffer(target_a);
                defer
                {
                    commandBuffer.Finish();
                    RenderManager.SubmitCommandBuffer(commandBuffer);
                ...
                commandBuffer.Viewport = .() { x = 0, y = 0, width = (uint16)width, height = (uint16)height };
                parameters.Blur = blur_vector;
                vertexbuffer.Render(commandBuffer, .Identity, blur_shader, scope GpuTexture[](target_b.TextureHandle), .White, state, sampler, parameters: .((uint8*)&parameters, sizeof(CrtParameters)));
            }
        }
        ...
        void RenderAccumulateCRT(RenderTexture texture0, RenderTexture texture1, RenderTexture target, int width, int height, float modulate)
        {
            ...
            var commandBuffer = RenderManager.AcquireCommandBuffer(target);
            defer
            {
                commandBuffer.Finish();
                RenderManager.SubmitCommandBuffer(commandBuffer);
            ...
            vertexbuffer.Render(commandBuffer, .Identity, accumulate_shader, scope GpuTexture[](texture0.TextureHandle, texture1.TextureHandle), .White, state, sampler, parameters: .((uint8*)&parameters, sizeof(CrtParameters)));
        }
        ...
        void RenderCopyCRT(RenderTexture source, RenderTexture destination, int width, int height)
        {
            RenderState state = .Opaque;
            SamplerDesc sampler = RenderManager.PointClamp;
            var commandBuffer = RenderManager.AcquireCommandBuffer(destination);
            defer
            {
                commandBuffer.Finish();
                RenderManager.SubmitCommandBuffer(commandBuffer);
            ...
            commandBuffer.ClearColorBuffer = false;
            commandBuffer.Viewport = .() { x = 0, y = 0, width = (uint16)width, height = (uint16)height };
            vertexbuffer.Render(commandBuffer, .Identity, copy_shader, scope GpuTexture[](source.TextureHandle), .White, state, sampler, parameters: .((uint8*)&parameters, sizeof(CrtParameters)));
        }
        ...
        void RenderBlendCRT(RenderTexture texture0, RenderTexture texture1, RenderTexture target, int width, int height, float modulate)
        {
            ...
            commandBuffer.Viewport = .() { x = 0, y = 0, width = (uint16)width, height = (uint16)height };
            parameters.Modulate = modulate_vector;
            vertexbuffer.Render(commandBuffer, .Identity, blend_shader, scope GpuTexture[](texture0.TextureHandle, texture1.TextureHandle), .White, state, sampler, parameters: .((uint8*)&parameters, sizeof(CrtParameters)));
        }
        ...
        public void Render(RenderTexture sourceTexture, int windowWidth, int windowHeight)
        {
            ...
            {
                RenderCommandBuffer commandBuffer = RenderManager.AcquireSwapchainCommandBuffer();
                defer
                {
                    commandBuffer.Finish();
                    RenderManager.SubmitCommandBuffer(commandBuffer);
                }
                commandBuffer.ClearColorBuffer = true;
                commandBuffer.Viewport = .() { x = 0, y = 0, width = (uint16)windowWidth, height = (uint16)windowHeight };
            }
            ...
            {
                commandBuffer.ClearColorBuffer = false;
                commandBuffer.Viewport = .() { x = (uint16)renderOffsetAndSize.x, y = (uint16)renderOffsetAndSize.y, width = (uint16)renderOffsetAndSize.z, height = (uint16)renderOffsetAndSize.w };
            }
            ...
            screenvertexbuffer.Render(commandBuffer, .Identity, crt_shader, scope GpuTexture[](accumulatetexture_a.TextureHandle, blurtexture_a.TextureHandle), .White, state, sampler, parameters: .((uint8*)&parameters, sizeof(CrtParameters)));
        }
