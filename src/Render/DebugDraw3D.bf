...
namespace GameCore
{
    class DebugDraw3D
    {
        ...
        private static void RenderText(RenderCommandBuffer commandBuffer, DebugText debugText, int shaderProgramIndex)
        {
            if (DebugFont == null)
            {
                return;
            ...
            DebugFont.RenderText(RenderManager.batchRenderer, RenderManager.GetShader(.Font), commandBuffer, .Zero, matrix, debugText.color, debugText.text, .Black, shaderProgramIndex, 0, null, modelViewMatrix);
        }

        public static void Render(RenderCommandBuffer commandBuffer, bool render = true, Camera camera = null)
        {
            if (!render)
            {
                Clear();
            ...
            if (camera != null)
                state.Depth.depth_compare = camera.DepthTest;
            if (debugSolidVertices.Count > 0)
            {
                var vertices = commandBuffer.TransientVertices(debugSolidVertices.Ptr, (uint32)(debugSolidVertices.Count * sizeof(DebugVertex)), vertexLayout);
                RenderManager.Draw(commandBuffer, shader, 0, vertices, default, (.)debugSolidVertices.Count, 0, .Identity, .One, .Zero, state: state);
            }
            if (debugVertices.Count > 0)
            {
                ...
                RenderManager.Draw(commandBuffer, shader, 0, vertices, default, (.)debugVertices.Count, 0, .Identity, .One, .Zero, state: state, lines: true);
            }
            if (debugCubes.Count > 0)
            {
                var storage = commandBuffer.Upload(debugCubes.Ptr, (uint64)(debugCubes.Count * sizeof(Vector4)));
                uint32 count = (uint32)debugCubes.Count / 2;
                RenderManager.Draw(commandBuffer, shader, 1, default, huge_index_buffer, count * 8, count * NUM_CUBE_INDICES, .Identity, .One, .Zero, state: state, storage: storage);
            }
            if (debug2DVertices.Count > 0)
            {
                var vertices = commandBuffer.TransientVertices(debug2DVertices.Ptr, (uint32)(debug2DVertices.Count * sizeof(DebugVertex)), vertexLayout);
                RenderManager.Draw(commandBuffer, shader, 2, vertices, default, (.)debug2DVertices.Count, 0, .Identity, .One, .Zero, state: .Alpha);
            }
            for (var text in debug2DTexts)
                RenderText(commandBuffer, text, 0);
            for (var text in debug3DTexts)
                RenderText(commandBuffer, text, 1);
            Clear();
        }

        public static void Finalize()
        {
            huge_index_buffer.Dispose();
            huge_index_buffer = .Null;
            Clear();
        }
