...
public class ShaderBuilder : ResourceBuilder
{
    ...
    public static bool IsSharedInclude(StringView path)
    {
        ...
        return fileName.Equals("gamecore.slang", .OrdinalIgnoreCase) || fileName.EndsWith(".slangh", .OrdinalIgnoreCase);
    }
    ...
    private ShaderCompiler SelectCompiler(ResourceBuilderPlatform platform)
    {
        ...
        case .Windows:
            return dx12Compiler;
        default:
            Error.Set(scope $"Shader compilation is not implemented for {platform}");
            return null;
        }
    }
    ...
    public bool NeedsBuild(StringView source, StringView output, StringView platformIncludeDirectory)
    {
        ...
        for (var dependency in visitedDependencies)
            delete dependency;
        visitedDependencies.Clear();
        if (!(File.GetLastWriteTimeUtc(output) case .Ok(let outputTime)))
            return true;
        var manifest = scope $"{source}.json";
        if (File.Exists(manifest) && (!(File.GetLastWriteTimeUtc(manifest) case .Ok(let manifestTime)) || manifestTime > outputTime))
            return true;
    ...
    private bool DependencyNeedsBuild(StringView path, DateTime outputTime)
    {
        ...
        for (var line in text.Split('\n'))
        {
            ...
            directive.TrimStart();
            if (directive.IsEmpty || (directive[0] != '"' && directive[0] != '<'))
                continue;
            char8 closing = directive[0] == '"' ? '"' : '>';
            int end = directive.IndexOf(closing, 1);
            if (end < 0)
            {
                Error.Set(scope $"Invalid include in {path}");
                return true;
            }
            ...
            if (DependencyNeedsBuild(includePath, outputTime))
                return true;
        }
        return false;
    }
    ...
    private bool CompileVariant(StringView source, JSONObject variant, ShaderCompiler compiler, DynMemStream package)
    {
        ...
        if (variant != null)
        {
            ...
            {
                Error.Set("Shader variant requires a nonempty name");
                return false;
            }
            if (variant.ContainsKey("defines"))
            {
                ...
                {
                    Error.Set("Shader variant defines must be an array");
                    return false;
                }
                for (int index = 0; index < defines.Count; ++index)
                {
                    ...
                    {
                        Error.Set("Shader defines must be strings");
                        return false;
                    }
        ...
        {
            Error.Set(scope $"Duplicate shader variant: {name}");
            return false;
        }
        package.Write<int32>((.)name.Length);
        package.TryWrite(.((uint8*)name.Ptr, name.Length));
        if (variant != null && variant.ContainsKey("compute"))
        {
            if (variant.ContainsKey("vertex") || variant.ContainsKey("fragment"))
            {
                Error.Set("Compute variants cannot contain graphics stages");
                return false;
            }
            if (!GetEntryPoint(variant, "compute", "computeMain", let computeEntry))
                return false;
            if (!compiler.CompileStage(source, computeEntry, .Compute,
                variantDefines, includeDirectories, stageBytecode, Error)) return false;
            package.Write<int32>(0);
            package.Write<int32>((.)stageBytecode.Count);
            package.TryWrite(stageBytecode);
            return true;
        }
        if (!GetEntryPoint(variant, "vertex", "vertexMain", let vertexEntry))
            return false;
        if (!GetEntryPoint(variant, "fragment", "fragmentMain", let fragmentEntry))
            return false;
    ...
    {
        Error.Clear();
        if (!source.EndsWith(".slang"))
        {
            Error.Set("Shader sources must be native .slang files");
            return false;
        }
        ConfigureIncludes(platformIncludeDirectory);
        ...
        for (int index = 0; index < count; ++index)
        {
            ...
            {
                Error.Set("Shader variant must be an object");
                return false;
            }
            if (!CompileVariant(source, variant, selectedCompiler, package))
                return false;
        }
        ...
        {
            Error.Set("Cannot create shader output directory");
            return false;
        }
        if (File.WriteAll(output, packageBytes) case .Err)
        {
            Error.Set("Cannot write shader package");
            return false;
        }
        return true;
    }
