...
public class Dx12ShaderCompiler : ShaderCompiler
{
        ...
        List<String> defines, List<String> includeDirectories, List<uint8> bytecode, String error)
    {
        ...
        { error.Set("Cannot create temporary shader file"); return false; }
        ...
        arguments.Append(stage == .Vertex ? " -stage vertex -o" : " -stage fragment -o");
        ...
        for (var define in defines) AppendArgument(arguments, scope $"-D{define}");
        ...
        { error.Set(scope $"Slang failed for {source}, {entryPoint} ({stage})"); return false; }
        if (File.ReadAll(temporaryPath, bytecode) case .Err)
        { error.Set("Cannot read compiled shader"); return false; }
        if (bytecode.Count < 4 || (bytecode.Count & 3) != 0 ||
            bytecode[0] != 'D' || bytecode[1] != 'X' || bytecode[2] != 'B' || bytecode[3] != 'C')
        { error.Set("Slang did not produce a DXIL container"); return false; }
        return true;
    }
}
